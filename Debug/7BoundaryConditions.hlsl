...
)
{
	...
	{*/
		...
		//	(gl_GlobalInvocationID.x == texResolution.x - 1 && gl_GlobalInvocationID.y == 0) ||
		//	(gl_GlobalInvocationID.x == 0 && gl_GlobalInvocationID.y == texResolution.y - 1) ||
		//	(gl_GlobalInvocationID.x == texResolution.x - 1 && gl_GlobalInvocationID.y == texResolution.y - 1))
		//{
		//	float3 zero = float3(0, 0, 0);
		//
		//	velocityTexture[gl_GlobalInvocationID] = zero;
		//	pressureTexture[gl_GlobalInvocationID] = zero;
		//	inkTexture[gl_GlobalInvocationID] = zero;
		//}
	//}
}