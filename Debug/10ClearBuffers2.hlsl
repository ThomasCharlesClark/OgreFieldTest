...
RWTexture3D<float> inkTextureFinal				: register(u1);
...
)
{
	if( gl_GlobalInvocationID.x < texResolution.x && gl_GlobalInvocationID.y < texResolution.y)
	{
		...
		inkTexture[gl_GlobalInvocationID] *= 0.92;
		inkTextureFinal[gl_GlobalInvocationID] *= 0.92;

		//inkTexture[gl_GlobalInvocationID] = float4(0, 0, 0, 0);
		//inkTextureFinal[gl_GlobalInvocationID] = float4(0, 0, 0, 0);
